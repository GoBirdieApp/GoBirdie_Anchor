/**
 * Calibrates the aerodynamic coefficients in `src/physics/aero.ts`.
 * Run with `npm run fit:aero` when the anchor data changes.
 *
 * Two stages, because fitting the dynamic model straight to the anchor table is
 * badly posed (one smooth curve pair against a 10x span of spin ratios, and the
 * optimiser trades apex error for carry error in ways that wreck flight shape):
 *
 *   1. Invert each anchor row on its own. Holding Cd and Cl constant through the
 *      flight makes carry and apex two equations in two unknowns, so every row
 *      yields the effective Cd/Cl of that shot plus its flight-mean spin ratio.
 *      Landing angle is left out of the inversion and becomes the check that the
 *      row is internally consistent at all.
 *   2. Fit the smooth Cd(S) and Cl(S) curves to those per-row coefficients. This
 *      stage needs no trajectories, so it is exact and instant.
 *
 * A verification pass then runs the full dynamic model against every row.
 *
 * OTTOM1, 18.08.2026
 */
...
/**
 * Only measured Trackman tour averages are calibrated against. The 1-/2-iron
 * placeholders and both specialty wedges were extrapolated by hand (their carry
 * sits on a straight line in ball speed, which cannot hold where carry scales with
 * v^2), so they are reported as diagnostics and re-derived from the fitted model.
 */
const PGA_FIT_CLUBS = [
  'Driver',
  '3-Wood',
...
/**
 * Carry rises with Cl and falls with Cd; apex rises with Cl and is almost blind to
 * Cd. Alternating one-dimensional bisections on that near-triangular system settle
 * quickly, and a shrinking local grid polishes the result.
 */
...
/** Cd curve: [dragBase, dragSpin, dragSpinExponent, dragSpeedExponent]. */
...
/** Cl curve: [liftMax, liftHalfSpinRatio, liftExponent]. */
...
/**
 * Stage 2 places coefficients at each shot's *mean* spin ratio, but the integrator
 * evaluates them instantaneously, and spin ratio climbs steeply as the ball slows.
 * This pass therefore refines the seeded curves against full trajectories, scoring
 * flight shape (apex, landing angle) at least as tightly as carry so the optimiser
 * cannot buy distance by ballooning the flight.
 */
/*
Residual weights, and the one judgement call in this script.

Stage 1 shows the anchor columns cannot all be true at once: matching a row's own
carry and apex with constant coefficients lands 4-10 deg shallower than the landing
angle printed beside them. Something has to lose, and weighting landing angle
tightly (as an earlier revision did) makes the optimiser buy flight shape with
distance, which is how the model ended up 14 yd short at driver ball speed.

Carry is therefore weighted hardest: it is the number players measure, the number
the profile is generated from, and the number every downstream gapping decision
depends on. Apex follows, since launch monitors pin it well. Landing angle is
scored loosest and is treated as a consistency check rather than a target.
*/
...
/**
 * Cd/Cl windows spanning the bag, from the stage 1 inversion cross-checked against
 * published modern-ball wind-tunnel data. Without these the optimiser flattens drag
 * against spin ratio, which is fine for the fitted rows but wrong for the high-spin
 * wedge shots the bands and validator have to reason about.
 */
